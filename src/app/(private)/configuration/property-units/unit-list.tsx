...
export default function PropertyUnitsList() {
  ...
  const { data, isLoading, isError, error } = useGetPropertyUnits(search);
...
function UnitCard({ unit }: { unit: any }) {
  ...
  return (
    <Card key={unit.id} className="flex flex-row gap-3 items-center">
      ...
      <CardFooter className="flex flex-row items-center gap-1 p-6">
        {!isPending && (
          <UpdateSheet open={open} setOpen={setOpen} old_data={unit} />
        )}
        <Button
          variant={"outline"}
          size={"icon"}
